  ...
  if($doIt) { 
    ...
    if (move_uploaded_file($source, $gui->dest)) {
      ...
      switch($args->importType) {
        case 'XML':
          ...
          break;
        case 'EXCEL':
          $pcheck_fn = "check_excel_tc_tsuite";
          $pimport_fn = "importTestCaseDataFromEXCEL";
          break;
      }
    ...
    if($gui->file_check['status_ok'] && $pimport_fn) {
        if ($args->importType == 'EXCEL'){
            ...
            $gui->file_check = $pimport_fn($db,$gui->dest,
                intval($args->container_id),intval($args->tproject_id),intval($args->userID),
                $opt);
            $gui->dest = str_replace('.xlsx','.xml',$gui->dest);
            $pimport_fn = "importTestCaseDataFromXML";
        }
    }
...
function initializeGui(&$dbHandler,&$argsObj)
{
  ...
  $dest_files = array('XML' => $dest_common . ".xml",'EXCEL' => $dest_common . ".xlsx");
