...
function select_exportType()
{
    var filename_val = document.getElementById('export_xml').export_filename.value;
    var select_val = document.getElementById('export_xml').exportType.value;
    if (select_val == 'XML'){
        filename_val = removeExtension(filename_val)+".xml";
    }
    else if (select_val == 'EXCEL'){
        filename_val = removeExtension(filename_val)+".xlsx";
    }
    document.getElementById('export_xml').export_filename.value = filename_val;
}
...
{if $gui->do_it eq 1}
  <form method="post" id="export_xml" enctype="multipart/form-data"
    ...
    <td>
  	...
  	<td>
  	<select name="exportType" onchange="javascript:select_exportType();">
